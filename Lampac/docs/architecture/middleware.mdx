...
<Steps>
  <Step title="ForwardedHeaders">
    Обработка заголовков от reverse proxy (`X-Forwarded-For`, `X-Forwarded-Proto`) с loopback-адресов и из сетей `KnownProxies`. Корректирует IP клиента и схему запроса. `X-Forwarded-Host` не обрабатывается: хост берётся из `Host`.
  </Step>
  <Step title="BaseMod">
