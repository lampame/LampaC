...
Включите модуль в `init.conf` или `init.yaml`:
...
В режиме copy длительность сегмента зависит от ключевых кадров исходного видео и может немного отличаться от `segment_seconds`. При перекодировании модуль создаёт ключевые кадры сам, поэтому длительность сегментов точнее.
...
| Параметр | По умолчанию | Назначение |
|---|---:|---|
| `segment_seconds` | `9` | Целевая длительность одного HLS/fMP4-сегмента в секундах. |
...
| `segment_diff` | `10` | Допуск выравнивания сегментов по временным меткам исходных ключевых кадров в режиме copy. При перекодировании не используется. Обычно менять не нужно. |
...
`conf_uids` задаёт отдельный pipeline для конкретного устройства. Параметры профиля не объединяются с основными настройками pipeline: неуказанные поля получают стандартные значения модуля. `enable`, `allowed_uids`, `inactiveMinutes`, `gst_version` и `PATH` остаются общими.
